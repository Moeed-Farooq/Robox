package robuxskin.getskin.com.perkox
...
class RoboxPerkoxModule(private val reactContext: ReactApplicationContext) :
  ...
  override fun getName(): String = "RoboxPerkoxModule"
  ...
  fun showOfferwall(options: ReadableMap?, promise: Promise) {
    ...
    try {
      val intent = Intent(activity, RoboxPerkoxOfferwallActivity::class.java)
      intent.putExtra(
        RoboxPerkoxOfferwallActivity.EXTRA_APP_ID,
        options?.getString("appId") ?: "",
      )
      intent.putExtra(
        RoboxPerkoxOfferwallActivity.EXTRA_SDK_KEY,
        options?.getString("sdkKey") ?: "",
      )
      intent.putExtra(
        RoboxPerkoxOfferwallActivity.EXTRA_PLAYER_ID,
        options?.getString("playerId") ?: "",
      )
      intent.putExtra(
        RoboxPerkoxOfferwallActivity.EXTRA_BETA,
  ...
  companion object {
    ...
    @Volatile
    var instance: RoboxPerkoxModule? = null
      private set
